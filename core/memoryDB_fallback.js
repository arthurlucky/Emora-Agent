/**
 * core/memoryDB_fallback.js
 * 
 * Fallback version if better-sqlite3 is not available.
 * Uses original JSON system but with enhanced API.
 */
...
export async function listSessions() {
  ...
  // Add sessions that exist in meta but not in filesystem
...
// ============================================================================
// MIGRATION UTILITY (simplified)
// ============================================================================

export async function migrateFromJSON() {
  console.log('🔄 Starting simplified migration from JSON...');
  console.log('⚠️  Note: Using enhanced JSON system (SQLite features simulated)');
  
  try {
    const sessions = await listSessions();
    console.log(`✅ Found ${sessions.length} sessions`);
    console.log(`💡 All data remains in JSON format with enhanced API`);
    console.log(`📁 Location: ${MEMORY_DIR}/`);
    
    return {
      migratedSessions: sessions.length,
      databaseType: 'JSON (enhanced)',
      notes: 'Using fallback mode with better-sqlite3-like API'
    };
  } catch (err) {
...
// Auto-detect if better-sqlite3 is available
let hasBetterSQLite = false;
try {
  await import('better-sqlite3');
  hasBetterSQLite = true;
  console.log('ℹ️  better-sqlite3 available, using full SQLite mode');
} catch {
  console.log('ℹ️  better-sqlite3 not available, using JSON fallback mode');
}
...
export default {
  ...
  searchHistory,
  migrateFromJSON,
  databaseType: hasBetterSQLite ? 'SQLite' : 'JSON (fallback)',
  hasBetterSQLite
};
