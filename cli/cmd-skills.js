...
function loadSkills() {
  ...
  return fs.readdirSync(SKILL_DIR, { withFileTypes: true })
    ...
    .map(d => {
      ...
      const hasMd     = fs.existsSync(path.join(SKILL_DIR, name, "skill.md"));
      const mdSize    = hasMd ? fs.statSync(path.join(SKILL_DIR, name, "skill.md")).size : 0;
      return { name, meta, hasScript, hasMd, mdSize };
    });
}
...
function printSkillDetail(skill) {
  ...
  const lines = [
    ...
    ["Author",      skill.meta.version || "EMORA Skill Factory"],
  ...
  if (skill.hasMd) {
    const md = fs.readFileSync(path.join(SKILL_DIR, skill.name, "skill.md"), "utf8");
